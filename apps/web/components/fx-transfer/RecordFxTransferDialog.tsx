  ...
  return (
    <Dialog.Root open={open} onOpenChange={(next) => (next ? onOpenChange(true) : cancel())}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="!fixed left-1/2 top-1/2 z-[71] max-h-[calc(100dvh_-_2rem)] w-[calc(100%_-_2rem)] max-w-3xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl border border-border bg-card p-5 text-card-foreground shadow-xl focus:outline-none sm:p-6"

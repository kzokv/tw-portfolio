...
test.describe("ai connector history responsive validation", () => {
  test("[ai connectors]: mobile History and Permissions layouts stay within viewport", async ({
    appShell,
    page,
  }, testInfo) => {
    ...
    await page.screenshot({
      fullPage: true,
      path: `test-results/ai-connectors-history-${testInfo.project.name}-validation.png`,
    });
    ...
    await page.screenshot({
      fullPage: true,
      path: `test-results/ai-connectors-detail-${testInfo.project.name}-validation.png`,
    });
    ...
    await page.screenshot({
      fullPage: true,
      path: `test-results/ai-connectors-permissions-${testInfo.project.name}-validation.png`,
    });
  });
});

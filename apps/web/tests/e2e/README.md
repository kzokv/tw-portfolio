...
- **Build**: E2E npm scripts build the shared test packages and `@tw-portfolio/web` before Playwright starts. If you run `npx playwright test` directly against these configs, build `@tw-portfolio/web` first so the standalone server exists.

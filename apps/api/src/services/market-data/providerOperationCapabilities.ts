...
const PROVIDER_CAPABILITY_DEFINITIONS: Record<string, ProviderCapabilityDefinition> = {
  ...
  "yahoo-finance-jp": {
    ...
    supportsRerun: true,
    supportsResolverModes: false,
    emptyMappingReason: "Yahoo Finance JP uses direct .T suffix normalization and no durable symbol mappings.",
    supportedActions: WRITE_PROVIDER_ACTIONS,
  },
  "finmind-tw": {
    supportsMappings: false,
  ...
  "twelve-data-jp": {
    ...
    supportsRerun: false,
    supportsResolverModes: false,
    emptyMappingReason: "Twelve Data JP is catalog metadata only in this console.",
    supportedActions: {
      sync_catalog: "checkbox",
      backfill_catalog_rows: "typed_preview",
      renew_evidence: "checkbox",
    ...
    disabledReasons: {
      repair_mapping: "JP v1 does not use durable provider-symbol mappings.",
      rerun_backfill: "Twelve Data JP is catalog metadata only; Yahoo Finance JP owns JP bar reruns.",
      reverify_mapping: "JP v1 does not use durable provider-symbol mappings.",
      revert_mapping: "JP v1 does not use durable provider-symbol mappings.",
    },
  },
  frankfurter: {
    supportsMappings: false,

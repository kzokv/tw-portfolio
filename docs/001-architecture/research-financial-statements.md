...
- Publication side: raw revised artifacts preserve the original filing publication timestamp and use their retrieval timestamp as the first-known revision publication instant when no richer revision timestamp is supplied. Canonical records reject retrieval timestamps earlier than the effective original or revision publication timestamp.
...
- Trusted upstream IFRS and official MOPS TIFRS taxonomy families use the same period- and concept-based statement classification; issuer extensions remain unmapped.

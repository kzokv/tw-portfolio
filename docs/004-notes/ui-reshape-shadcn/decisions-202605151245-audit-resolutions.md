...
| 5 | Recompute action | ~~**Move to topbar overflow + per-ticker row action.** Global "Recompute all" lives in avatar menu and ⌘K. Per-ticker recompute moves into holdings-row action menu. Delete `RecomputeCard.tsx`.~~ **Amended 2026-05-16** per `phase-3-spec-202605161110-shell-decomp.md` §12 A2 — avatar menu carries NO Recompute entry (Decision #11 made the menu identity-only); ⌘K palette's `action.recompute.all` is the only global entry point and uses a shadcn `AlertDialog` (NOT `window.confirm`). Per-page Recompute button is retained on `/portfolio` only (mockup 13). Per-ticker row action retained per the original decision. | `/transactions`, `/portfolio`, `/dashboard` |

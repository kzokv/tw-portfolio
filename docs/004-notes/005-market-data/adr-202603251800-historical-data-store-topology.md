...
**Date:** 2026-03-25 (created), 2026-04-01 (frozen)
**Status:** Frozen — promoted to evergreen doc at [`docs/market-data-platform.md`](../market-data-platform.md) on 2026-04-01.
...
> **This document is now frozen.** The living architecture reference is [`docs/market-data-platform.md`](../market-data-platform.md). This file preserves the original decisions and the state of the system as of 2026-04-01 when it was promoted. Do not update this file — update the evergreen doc instead.
...
### Ingest pipeline: job chain and ticket ownership

Each job in the daily ingest chain is owned by a specific ticket. Implementation status as of 2026-04-01:

| Job | Name | Ticket | Status | Chains from |
|-----|------|--------|--------|-------------|
| 1 | Catalog Sync | KZO-83 (impl) + KZO-130 (cron scheduling) | **Done** | pg-boss cron `30 17 * * 1-5` |
...
│  │  │   └─ FinMind TaiwanStockDividend → dividend evts  │          │      │
...
│  │                                                                           │      │
│  │  Pages:                          Hooks:                                   │      │
│  │  ├─ /portfolio (holdings)        ├─ useEventStream (SSE, always-on)       │      │
│  │  ├─ /portfolio/[ticker]          ├─ useMutations (recompute/backfill)     │      │
│  │  ├─ /settings (monitored)        └─ useAuth (session cookies)             │      │
│  │  └─ /login                                                                │      │
│  │                                                                           │      │
│  │  State:                          Missing (KZO-132):                       │      │
│  │  ├─ Zustand stores               └─ daily_refresh SSE event handling      │      │
│  │  ├─ React Query cache                                                     │      │
...
│  │  │  ├─ upserts.ts           shared upsertDailyBars/DividendEvents   │   │      │

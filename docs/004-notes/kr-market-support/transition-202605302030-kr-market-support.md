...
- Supported KR catalog instruments are KRX common stocks, preferred stocks, REITs, and ETFs.
...
|---|---|---|
| Catalog | Twelve Data Basic/free | `/stocks` and `/etf` reference endpoints can enumerate KRX instruments by exchange. |
...
- `TwelveDataKrCatalogProvider` fetches `/stocks?exchange=KRX` and `/etf?exchange=KRX`, validates `mic_code = "XKRX"`, filters unsupported rows, and delegates metadata/search to the Yahoo KR provider.
...
- No sector/GICS filter in the KR instrument catalog sheet, because the no-paid KRX catalog path does not provide reliable sector data.

...
|-----------|-------|-------------|--------|
| `root:local` | Root: local | `.env.local` | `rootLocalSchema` |
...
| `e2eEnvSchema` | Test-specific (future) | `GOOGLE_OAUTH_REFRESH_TOKEN` + test config vars — deferred; explicitly out of KZO-103 scope |

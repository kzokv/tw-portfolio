...
|--------|---------|---------|
| `envSchema` | Root app config | Add `DEPLOY_ENV`, `NEXT_PUBLIC_AUTH_MODE`, `NEXT_PUBLIC_API_BASE_URL`; absorb `webEnvSchema` |
| `dockerCloudSchema` | Unified dev + prod Docker | Merge `dockerDevSchema` + `dockerProdSchema`; add `DEPLOY_ENV`; domain defaults driven by `DEPLOY_ENV` |
| `dockerLocalSchema` | Docker local (no tunnel) | Stays as-is (no `CLOUDFLARE_TUNNEL_TOKEN`, no `PUBLIC_DOMAIN_*`) |
| `e2eEnvSchema` | Test-specific (new) | `GOOGLE_OAUTH_REFRESH_TOKEN` + test config vars |
| ~~`webEnvSchema`~~ | Removed | Folded into `envSchema` |
| ~~`dockerDevSchema`~~ | Removed | Merged into `dockerCloudSchema` |
| ~~`dockerProdSchema`~~ | Removed | Merged into `dockerCloudSchema` |
...
|------|------------|-------------|
| Port uniqueness | `validatePortConflicts()` | API_PORT, WEB_PORT, DB_PORT, REDIS_PORT must all differ |
| dev_bypass restriction | `validatePortConflicts()` | `AUTH_MODE=dev_bypass` only allowed when `NODE_ENV=development` |
| OAuth required vars | `validatePortConflicts()` | When `AUTH_MODE=oauth`: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, `SESSION_SECRET` all required |
...
flowchart TD
    ...
    G --> H
    H --> I{Validation}
    I --> J[validatePortConflicts]
    I --> K[validateHostConsistency]
    I --> L[validateCookieConfig]
    J --> M{AUTH_MODE?}
    M -->|oauth| N[Check required<br/>OAuth vars present]
    M -->|dev_bypass| O{NODE_ENV =<br/>development?}
    O -->|No| P[ERROR: dev_bypass<br/>only in development]
    O -->|Yes| Q[OK]
    N --> Q
    K --> Q
    L --> Q

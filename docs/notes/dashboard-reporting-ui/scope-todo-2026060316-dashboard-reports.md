...
- [ ] Commit F: fix scoped report correctness for Daily Review, Portfolio Report, and Market Report with `scope=TW` and other single-market scopes. Partial: deployed Chrome validation found TW Market Report could still hit the client timeout; follow-up backend work indexed scoped store market resolution so irrelevant dividend-event/instrument rows no longer cause repeated full-store scans before report generation.
- [ ] Commit F: ensure scoped reports filter holdings, trades, quotes, daily bars, dividends, and performance contributors consistently by market. Partial: existing focused API tests cover TW/AU scope behavior, scoped aggregate performance, market-aware daily bars, and scoped dividend events; the latest fix preserves behavior while optimizing the scoped filter path.

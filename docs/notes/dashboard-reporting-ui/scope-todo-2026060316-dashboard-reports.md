...
- [x] Commit C: implement hybrid dashboard/report performance read path that may use reliable snapshot Market Value but never uses FX-moving snapshot cost as Book Cost. The performance series overlay is implemented in `dashboardReportingCurrency.ts` and covered by current-head unit/integration tests; report/dashboard summary totals and detail rows still need the separate Commit B migration.
...
- [x] Commit C: update Return card to plot Return % from corrected daily performance points.
...
- [x] Commit D: implement native price/value progressive disclosure for dashboard/report holding cards and rows when reporting currency differs from native currency.
...
- [x] Commit F: fix scoped report correctness for Daily Review, Portfolio Report, and Market Report with `scope=TW` and other single-market scopes. Current evidence covers TW/AU scoped API behavior and deployed TW report recovery after the indexed scoped-store filtering fix.
- [x] Commit F: ensure scoped reports filter holdings, trades, quotes, daily bars, dividends, and performance contributors consistently by market. Focused API coverage includes TW/AU scope behavior, scoped aggregate performance, market-aware daily bars, upcoming dividend scoping, same-ticker market scoping, and ambiguous-snapshot fallback.
...
- [ ] Commit I: update UI labels, tooltips, data-health copy, and i18n for Book Cost, FX-Translated Cost, Return %, stale data, and missing FX. Partial: report UI now uses `Book Cost` labels consistently, but FX-Translated Cost, missing-FX/incomplete copy, and i18n extraction remain open.
...
- [x] 2026-06-09 current-head report diagnostics and Book Cost label follow-up:
  - Added `ReportDiagnosticsDto` to shared report DTOs and returned diagnostics from Daily Review, Portfolio Report, and Market Report. Diagnostics include selected scope, reporting currency, requested/as-of date, last valuation date, stale marker, missing/stale quote counts, missing-FX count, and report row counts.
  - Updated report UI terminology from legacy `Cost basis` labels to `Book Cost` in summary cards, desktop holdings tables, mobile rows, and detail sheets.
  - Focused API: `npm --prefix apps/api exec vitest run test/integration/reports.integration.test.ts -- --no-file-parallelism` passed: 13 tests.
  - Focused web: `npm exec vitest run test/components/reports/ReportsClient.test.tsx test/features/reports/hooks/useReportData.test.tsx` from `apps/web` passed: 12 tests.
  - Targeted lint: `npx eslint apps/api/src/services/reports.ts apps/api/test/integration/reports.integration.test.ts apps/web/components/reports/ReportsClient.tsx apps/web/test/components/reports/ReportsClient.test.tsx apps/web/test/features/reports/hooks/useReportData.test.tsx libs/shared-types/src/index.ts` passed.
  - Targeted typecheck: `npx tsc --noEmit -p apps/api/tsconfig.json --pretty false && npx tsc --noEmit -p apps/web/tsconfig.json --pretty false` passed after rebuilding ignored local shared-types `dist`.
  - `git diff --check` passed.
  - Additional independent audit runs passed on current head: API report/dashboard performance focused tests and web report/dashboard/settings/ticker/portfolio cache focused tests. Remaining unchecked items are intentional: summary/detail Book Cost migration, FX-Translated Cost/incomplete diagnostics, composite-calendar stale handling, MCP advice-ready semantic payloads, and missing E2E/browser coverage.
...
- [x] 2026-06-09 focused Commit B/C foundation validation after dashboard/report performance overlay:
  ...
  - Updated the reports integration assertion for the all-market synthetic performance fallback so it verifies `getDailyBarsForTickerMarket()` is used, matching the market-aware production read path and fixing the stale CI expectation that still watched `getDailyBarsForTickers()`.

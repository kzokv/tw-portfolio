...
- [ ] Commit E: add API memory/Postgres tests and web tests for preference validation, merge semantics, selected preset persistence, show/hide, reorder, and reset/default behavior. Partial: API HTTP validation/round-trip coverage and dashboard component hydration/PATCH/show-hide coverage pass; dedicated memory/Postgres, reorder, and reset/default assertions remain pending.

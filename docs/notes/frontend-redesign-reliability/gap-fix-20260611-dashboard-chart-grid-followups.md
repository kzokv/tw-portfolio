...
Historical live validation before this follow-up showed a truthfulness gap on non-TWD portfolios; the 2026-06-12 deployed branch validation recorded below confirms the dashboard USD/KRW chart gap was resolved for the representative portfolio:
...
- 2026-06-11: Live Chrome validation before deploying this branch reproduced the USD/KRW dashboard chart failure on Vakwen Dev. This was historical pre-deploy evidence and was superseded by the 2026-06-12 deployed validation below.
...
- 2026-06-12: Deployed `e90ea554` to Vakwen Dev and reran Chrome validation in the existing logged-in tab through the Codex Chrome Extension. Dashboard TWD, USD, and KRW all rendered Portfolio Trend and Return % charts after waiting for refresh completion. Exact reconciliation: TWD hero `NT$20,413,796.21` equals market strip `NT$11,567,500 + NT$5,363,678.61 + NT$3,482,617.60`; USD hero `$646,490.43` matches market strip within one-cent rounding; KRW hero `₩985,192,476.96` equals market strip `₩558,259,117.50 + ₩258,853,359.46 + ₩168,080,000`. Reports Market Report with all-market USD rendered `reports-performance-chart` and `reports-market-summary-chart` and showed exact inline values. Portfolio showed the requested `Dashboard Top Holdings` / `Portfolio Holdings` style switch, sticky first column, Avg Cost, and Unit P&L. Ticker `2330` showed market value, unrealized P&L, account breakdown, and range/timeline controls. The session was restored to TWD.
- 2026-06-12: Team cross-check found and local follow-up fixed the ticker reporting-currency disclosure gap. Ticker overview and Account breakdown now show the resolved reporting currency used by account contribution values, and the ticker header routes reporting-currency changes to global Quick Actions. Focused validation passed in the main scope note: `npx eslint 'apps/web/app/tickers/[ticker]/TickerHistoryClient.tsx' apps/web/features/dashboard/i18n.ts apps/web/lib/i18n/types.ts apps/web/test/app/tickers/TickerHistoryClient.test.tsx`; `npx vitest run test/app/tickers/TickerHistoryClient.test.tsx` from `apps/web` (`10` tests passed); `npx tsc --noEmit -p apps/web/tsconfig.json`.

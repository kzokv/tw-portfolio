...
- 2026-06-12: Team cross-check found one remaining ticker UI disclosure gap: ticker account contribution values were reporting-currency values, but the page did not visibly show the resolved reporting currency or provide the agreed Quick Actions handoff near the ticker overview/account breakdown. Local follow-up now shows `Reporting {currency}` in the ticker header and Account breakdown card, with a Quick Actions button for changing the global reporting currency. Focused validation is tracked in the main scope note.

...
5. Remember persistence semantics: top-level keys merge, `null` deletes, and nested objects are replaced wholesale except special-cased keys such as `cardOrder`. If a feature needs partial nested patches, update both Memory and Postgres persistence paths.

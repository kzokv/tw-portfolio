...
The npm scripts pin both `NEXT_PUBLIC_AUTH_MODE` and `NEXT_PUBLIC_API_BASE_URL` because Next.js inlines `NEXT_PUBLIC_*` at build time — see `.claude/rules/`-adjacent context in `docs/001-architecture/web-frontend.md` § "Build-Time vs Runtime Variables". Running `npm run build -w @tw-portfolio/web` without these env vars will produce a bundle that points at the wrong API URL.
...
**Before debugging the test or the source**, rerun `npm run build -w @tw-portfolio/web` (or use the suite's npm wrapper) and re-run.
...
- If you must call `npx playwright test` directly (faster iteration on a single spec), explicitly `npm run build -w @tw-portfolio/web` between source edits.

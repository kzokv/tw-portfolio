...
- A route may skip the global `router.refresh()` context-switch fallback only when every context-sensitive client on that route consumes the shared context refresh signal and clears or replaces prior-owner state. Scope such exceptions by route; do not remove the global fallback for unrelated server-seeded routes. Test both sides: the signal-aware route must refresh without remounting, while a non-signal route must retain the server refresh fallback.

...
**How to apply:** Only create npm script wrappers for scripts that take no required arguments or have simple fixed flag variants (e.g., `docker:validate` wraps `validate-local.sh` with no args, `docker:validate:teardown` wraps it with `--teardown`). For scripts with required positional args, document direct invocation instead.
